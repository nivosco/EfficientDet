...
def build_wBiFPN(features, num_channels, id, freeze_bn=False):
    if id == 0:
        ...
        P6_td = layers.Activation(lambda x: tf.nn.relu(x))(P6_td)
        ...
        P5_td = layers.Activation(lambda x: tf.nn.relu(x))(P5_td)
        ...
        P4_td = layers.Activation(lambda x: tf.nn.relu(x))(P4_td)
        ...
        P3_out = layers.Activation(lambda x: tf.nn.relu(x))(P3_out)
        ...
        P4_out = layers.Activation(lambda x: tf.nn.relu(x))(P4_out)
        ...
        P5_out = layers.Activation(lambda x: tf.nn.relu(x))(P5_out)
        ...
        P6_out = layers.Activation(lambda x: tf.nn.relu(x))(P6_out)
        ...
        P7_out = layers.Activation(lambda x: tf.nn.relu(x))(P7_out)
...
class BoxNet(models.Model):
    def __init__(self, width, depth, num_anchors=9, separable_conv=True, freeze_bn=False, detect_quadrangle=False, **kwargs):
        ...
        #             for i in range(depth)]
        self.relu = layers.Lambda(lambda x: tf.nn.relu(x))
